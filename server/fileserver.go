...
func uploadFile(res http.ResponseWriter, req *http.Request) {
	...
	for _, fheaders := range req.MultipartForm.File {
		for _, hdr := range fheaders {
			...
			err := os.MkdirAll(filepath.Join(viper.GetString("file-server-path"), path), 0755)
			if err != nil {
	...
	writeResponse(res, req, http.StatusAccepted, &Response{fmt.Sprintf("Uploaded Files: %v\nTotal Bytes Written: %v", uploadedFileNames, totalBytesWritten)})
